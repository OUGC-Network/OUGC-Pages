...
';
        ...
        if ($mybb->request_method == 'post') {
            ...
            $query = $db->simple_select(
                'ougc_pages',
                'pid',
                'url=\'' . $db->escape_string($url) . '\'' . ($add ? '' : ' AND pid!=\'' . $mybb->get_input('pid', 1) . '\''),
                array('limit' => 1)
            );
        ...
        $form_container->output_row(
            $lang->ougc_pages_form_category,
            $lang->ougc_pages_form_category_desc,
            $ougc_pages->generate_category_select('category', $mybb->get_input('cid', 1))
        );
        $form_container->output_row(
            $lang->ougc_pages_form_name . ' <em>*</em>',
            $lang->ougc_pages_form_name_desc,
            $form->generate_text_box('name', $mybb->get_input('name'))
        );
        $form_container->output_row(
            $lang->ougc_pages_form_description . ' <em>*</em>',
            $lang->ougc_pages_form_description_desc,
            $form->generate_text_box('description', $mybb->get_input('description'))
        );
        $form_container->output_row(
            $lang->ougc_pages_form_url . ' <em>*</em>',
            $lang->ougc_pages_form_url_desc,
            $form->generate_text_box('url', $mybb->get_input('url'))
        );

        $form_container->output_row(
            $lang->ougc_pages_form_groups,
            $lang->ougc_pages_form_groups_desc,
            $form->generate_group_select(
                'groups[]',
                explode(',', $mybb->get_input('groups', MyBB::INPUT_STRING)),
                array('id' => 'groups', 'multiple' => true, 'size' => 5)
            ),
            '',
            array(),
            array('id' => 'row_groups')
        ...
        $form_container->output_row(
            $lang->ougc_pages_form_wol,
            $lang->ougc_pages_form_wol_desc,
            $form->generate_yes_no_radio('wol', $mybb->get_input('wol', 1))
        );
        $form_container->output_row(
            $lang->ougc_pages_form_visible,
            $lang->ougc_pages_form_visible_desc,
            $form->generate_yes_no_radio('visible', $mybb->get_input('visible', 1))
        );
        $form_container->output_row(
            $lang->ougc_pages_form_wrapper,
            $lang->ougc_pages_form_wrapper_desc,
            $form->generate_yes_no_radio('wrapper', $mybb->get_input('wrapper', 1))
        );
        $form_container->output_row(
            $lang->ougc_pages_form_php,
            $lang->ougc_pages_form_php_desc,
            $form->generate_yes_no_radio('php', $mybb->get_input('php', 1))
        );
        $form_container->output_row(
            $lang->ougc_pages_form_init,
            $lang->ougc_pages_form_init_desc,
            $form->generate_yes_no_radio('init', $mybb->get_input('init', 1))
        );
        $form_container->output_row(
            $lang->ougc_pages_form_disporder,
            $lang->ougc_pages_form_disporder_desc,
            $form->generate_text_box(
                'disporder',
                $mybb->get_input('disporder', 1),
                array('style' => 'text-align: center; width: 30px;" maxlength="5')
            )
        );
        $form_container->output_row(
            $lang->ougc_pages_form_template,
            $lang->ougc_pages_form_template_desc,
            $form->generate_text_area(
                'template',
                $mybb->get_input('template'),
                array('rows' => 5, 'id' => 'template', 'class' => '', 'style' => 'width: 100%; height: 500px;')
            )
        );
        ...
        $page->output_confirm_action(
            $ougc_pages->build_url(array('action' => 'delete', 'pid' => $mybb->get_input('pid', 1)))
        );
        ...
        $form_container->output_row(
            ...
            $form->generate_yes_no_radio('ignore_version', $mybb->get_input('ignore_version', 1))
        );
        ...
        if (!$count) {
            ...
            $query = $db->simple_select(
                ...
                'cid=\'' . (int)$category['cid'] . '\'',
                array(
                    'limit_start' => $ougc_pages->query_start,
                    'limit' => $ougc_pages->query_limit,
                    'order_by' => 'disporder',
                )
            );
            ...
            while ($pages = $db->fetch_array($query)) {
                ...
                $table->construct_cell(
                    '<a href="' . $edit_link . '"><strong>' . $pages['name'] . '</strong></a> <span style="font-size: 90%">(' . $ougc_pages->build_page_link(
                        $lang->ougc_pages_view_page,
                        $pages['pid']
                    ) . ')</span>'
                );
                $table->construct_cell(
                ...
                $table->construct_cell(
                    '<a href="' . $ougc_pages->build_url(
                        array('action' => 'update', 'pid' => $pages['pid'], 'my_post_key' => $mybb->post_code)
                    ) . '"><img src="styles/default/images/icons/bullet_o' . (!$pages['visible'] ? 'ff' : 'n') . '.png" alt="" title="' . (!$pages['visible'] ? $lang->ougc_pages_form_disabled : $lang->ougc_pages_form_visible) . '" /></a>',
                    array('class' => 'align_center')
                );
                ...
                $popup->add_item(
                    $lang->ougc_pages_form_export,
                    $ougc_pages->build_url(
                        array('action' => 'export', 'pid' => $pages['pid'], 'my_post_key' => $mybb->post_code)
                    )
                );
                $popup->add_item(
                    $lang->delete,
                    $ougc_pages->build_url(array('action' => 'delete', 'pid' => $pages['pid']))
                );
            ...
            $form->output_submit_wrapper(
                array(
                    $form->generate_submit_button($lang->ougc_pages_button_disponder),
                    $form->generate_reset_button($lang->reset),
                )
            );
            $form->end();
    ...
    if ($mybb->request_method == 'post') {
        ...
        $query = $db->simple_select(
            ...
            'cid',
            'url=\'' . $db->escape_string($url) . '\'' . ($add ? '' : ' AND cid!=\'' . $mybb->get_input('cid', 1) . '\''),
            array('limit' => 1)
        );
    ...
    $form = new Form(
        $ougc_pages->build_url(($add ? 'action=add' : array('action' => 'edit', 'cid' => $category['cid']))), 'post'
    );
    ...
    $form_container->output_row(
        $lang->ougc_pages_form_breadcrumb,
        $lang->ougc_pages_form_breadcrumb_desc,
        $form->generate_yes_no_radio('breadcrumb', $mybb->get_input('breadcrumb', 1))
    );
    $form_container->output_row(
        $lang->ougc_pages_form_wrapucp,
        $lang->ougc_pages_form_wrapucp_desc,
        $form->generate_yes_no_radio('wrapucp', $mybb->get_input('wrapucp', 1))
    );
    ...
    $page->output_confirm_action(
        $ougc_pages->build_url(array('action' => 'delete', 'cid' => $mybb->get_input('cid', 1)))
    );
} elseif ($mybb->get_input('action') == 'update') {
    if (!($category = $ougc_pages->get_category($mybb->get_input('cid', 1)))) {
        $ougc_pages->redirect($lang->ougc_pages_error_invalidcategory, true);
    ...
    if (!$count) {
        ...
        while ($category = $db->fetch_array($query)) {
            ...
            $table->construct_cell(
                '<a href="' . $manage_link . '"><strong>' . $category['name'] . '</strong></a> <span style="font-size: 90%">(' . $ougc_pages->build_category_link(
                    $lang->ougc_pages_view_page,
                    $category['cid']
                ) . ')</span>'
            );
            ...
            $table->construct_cell(
                '<a href="' . $ougc_pages->build_url(
                    array('action' => 'update', 'cid' => $category['cid'], 'my_post_key' => $mybb->post_code)
                ) . '"><img src="styles/default/images/icons/bullet_o' . (!$category['visible'] ? 'ff' : 'n') . '.png" alt="" title="' . (!$category['visible'] ? $lang->ougc_pages_form_disabled : $lang->ougc_pages_form_visible) . '" /></a>',
                array('class' => 'align_center')
            );
            ...
            $popup->add_item(
                $lang->delete,
                $ougc_pages->build_url(array('action' => 'delete', 'cid' => $category['cid']))
            );
            $table->construct_cell($popup->fetch(), array('class' => 'align_center'));
